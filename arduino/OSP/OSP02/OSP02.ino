...
IPAddress local_IP(192, 168, 1, 44); // Static IP address192.168.100.164
...
void loop() 
{
  ...
  d512 = node.getResponseBuffer(0);
  node.clearResponseBuffer();
  
  node.readHoldingRegisters(1, 1); 
  Serial.print("d25 : ");
  Serial.println(node.getResponseBuffer(0));
  d25 = node.getResponseBuffer(0);
  node.clearResponseBuffer();
  
  node.readHoldingRegisters(2, 1); 
  Serial.print("d515 : ");
  Serial.println(node.getResponseBuffer(0));
  d515 = node.getResponseBuffer(0);
  node.clearResponseBuffer();
  
  delay(500);
  led_blue();
  
  rssi.toCharArray(i_rssi, 16);client.publish("OSP02/rssi", i_rssi);
  d512.toCharArray(i0, 16);client.publish("OSP02/D512", i0);
  d25.toCharArray(i1, 16);client.publish("OSP02/D25", i1);
  d515.toCharArray(i2, 16);client.publish("OSP02/D515", i2);
